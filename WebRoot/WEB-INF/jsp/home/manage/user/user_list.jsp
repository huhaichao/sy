...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="usersy/list.do" method="post" name="Form" id="Form">
			<table>
				<tr>
					<td>
						<span class="input-icon">
							<input autocomplete="off" id="nav-search-input" type="text" name="T_SY_USERNAME" id="T_SY_USERNAME" value="${pd.T_SY_USERNAME }" placeholder="这里输入关键词" />
								...
								<td style="width: 30px;" class="center">
									<div class='hidden-phone visible-desktop btn-group'>
										...
										<ul class="dropdown-menu dropdown-icon-only dropdown-light pull-right dropdown-caret dropdown-close">
											<c:if test="${QX.edit == 1 }">
											<li><a style="cursor:pointer;" title="编辑" onclick="edit('${var.T_SY_USER_ID}');" class="tooltip-success" data-rel="tooltip" title="" data-placement="left"><span class="green"><i class="icon-edit"></i></span></a></li>
											</c:if>
											<c:if test="${QX.del == 1 }">
											...
											<%-- <li><a style="cursor:pointer;" title="查看" onclick="edit('${var.T_SY_USER_ID}');" class="tooltip-success" data-rel="tooltip" title="" data-placement="left"><span class="green"><i class="icon-eye-open"></i></span></a></li> --%>
											<c:if test="${QX.FX_QX == 1 }">
											<li><a style="cursor:pointer;" title="发送电子邮件" onclick="sendEmail('${var.T_SY_MAIL}');" class="tooltip-success" data-rel="tooltip" title="" data-placement="left"><span class="green"><i class='icon-envelope-alt'></i></span></a></li>
											</c:if>
											<c:if test="${QX.FW_QX == 1 }">
											<li><a style="cursor:pointer;"  title="发送短信" onclick="sendSms('${var.T_SY_PHONE}');" class="tooltip-warning" data-rel="tooltip" title="" data-placement="left"><span class="blue"><i class='icon-envelope'></i></span></a></li>
											</c:if>
										</ul>
										</div>
									</div>
		...
		<table style="width:100%;">
			<tr>
				<td style="vertical-align:top;">
					...
					<a class="btn btn-small btn-info" onclick="recycle();">回收站</a>
				</td>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>usersy/goAdd.do';
			 diag.Width = 700;
			 diag.Height = 700;
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>usersy/deleteUpd.do?T_SY_USER_ID="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>usersy/goEdit.do?T_SY_USER_ID='+Id;
			 diag.Width = 700;
			 diag.Height = 700;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function recycle(){
			 ...
			 diag.Drag=true;
			 diag.Title ="回收站";
			 diag.URL = '<%=basePath%>usersy/goRecycle.do';
			 diag.Width = 1200;
			 diag.Height = 700;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>usersy/deleteAllUpd.do?tm='+new Date().getTime(),
						    	data: {USER_IDS:str},
								dataType:'json',
